...
onready var attack1 = get_node("PlayerBox/HBoxContainer/Attack1")
onready var attack2 = get_node("PlayerBox/HBoxContainer/Attack2")
onready var attack3 = get_node("PlayerBox/HBoxContainer2/Attack3")
onready var attack4 = get_node("PlayerBox/HBoxContainer2/Attack4")
...
func _ready() -> void:
	...
	print (PlayerData.enemy_pokemon_info[0]["id"],"THIS IS THE ENEMY ID IN BATTLE")
	...
	if len(move_player) == 1:
		attack1.text = (move_player[0]["ename"])
	elif len(move_player) == 2:
		attack1.text = (move_player[0]["ename"])
		attack2.text = (move_player[1]["ename"])
	elif len(move_player) == 3:
		attack1.text = (move_player[0]["ename"])
		attack2.text = (move_player[1]["ename"])
		attack3.text = (move_player[2]["ename"])
	else:
		attack1.text = (move_player[0]["ename"])
		attack2.text = (move_player[1]["ename"])
		attack3.text = (move_player[2]["ename"])
		attack4.text = (move_player[3]["ename"])
	if PlayerData.catching_pokemon == true:
		if PlayerData.enemy_pokemon_info[0]["base"]["HP"] < 50:
			...
			$Label.text = "The savage pokemon has escaped"
	...
	if state == "lost":
		...
		if PlayerData.player_test["Pokedex"][info_player_position]["HP"] == 0:
			...
			if (x+1) < len(pokemon_):
				...
				get_tree().change_scene("res://Scenes/Battle.tscn") #cambiar por get scene for real
			else:
	 ...
	 # si quedan mas pokemn seguir la pelea
		...
		level_up(PlayerData.player_test["Pokedex"][info_player_position]["Level"])
		if PlayerData.is_colliding_with == 2:
...
func enemy_attacks()-> bool:
	...
	var damage2 = damage_to_player(number)
...
func manage_time():
	...
	$Timer.start()
	if STATES.PLAYER==true:
		pass
	elif STATES.ENEMY==true:
		$menu.visible=false
		$menu_movements.visible=false

func attack_pp(number:int):
	var box = get_node("PlayerBox/HBoxContainer3")
	var PP = get_node("PlayerBox/HBoxContainer3/Label")
	var Type = get_node("PlayerBox/HBoxContainer3/Label2")
	box.visible = true
	var current_pp = PlayerData.player_test["Pokedex"][info_player_position]["Movements"][selected_attack]["pp"]
	PP.text = "PP" + String(current_pp) + " / " + String(PlayerData.get_max_movement_pp(move_player[number]))
	Type.text = String(move_player[number]["type"])
...
func _on_Fight_pressed() -> bool:
	print (name_pokemon_player)
	print (move_player)
	#var damage = move_player[selected_attack]["power"]
	#if damage == null:
		#damage = 0
	var damage = damage_to_enemy()
...
func _on_Fight1_pressed() -> void:
	if $PlayerBox.visible == true:
		$PlayerBox.visible = false
	else:
		$PlayerBox.visible = true
...
func _on_Pokemon_pressed() -> void:
	get_tree().change_scene("res://Scenes/UI components/Pokemon_info_box_.tscn")
...
func _on_menu_selected_by_key(value) -> void:
	...
	if selected_item == "Fight":
		$Fight_box/HBoxContainer/Fight1.pressed = true
		$menu_movements.visible = true
		$menu_movements.set_process_unhandled_input(true)
...
func _on_menu_movements_selected_by_key(value) -> void:
	if value>=len(player_attacks):
		pass
	elif value==null:
		pass
	else:
		selected_attack = value
	$PlayerBox/HBoxContainer3/Fight.pressed = true
	if value != null and Input.is_action_pressed("ui_accept"):
		selected = true
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("pause") and $menu_movements.visible == true:
		...
		$menu_movements.visible = false
		$menu_movements.set_process_unhandled_input(false)
	if event.is_action_pressed("ui_focus_next") and selected_attack!=null and selected == true and STATES.PLAYER == true:
		_on_Fight_pressed()  #NECESITA UNA CONDICION DE MAS PARA QUE NO SE PUEDA DAR CLICK CUANDO ES EL TURNO DEL ENEMIGO
		#$menu_movements.visible = false


func _on_Timer_timeout():
	$someoneplays.text= "NEXT POKEMON"
	if STATES.PLAYER==true:
		$menu.visible=true
	elif STATES.ENEMY==true:
		$menu.visible=false
