...
var directiony = [Vector2(816,64),Vector2(816,128),Vector2(816,192)]
var pressed = 0 setget set_pressed

func _ready() -> void:
	$menu/Yes.text = PlayerData.player_test["name"] + "'s PC" 
	$menu/No.text = "Bill's PC"
	$menu/No2.text = "Oak's PC"
...
func input_event_entered():
	if $selector.rect_position == directiony[0]:
		...
		self.pressed = 1
		print ("NO")
	elif $selector.rect_position == directiony[2]:
		self.pressed = 2
